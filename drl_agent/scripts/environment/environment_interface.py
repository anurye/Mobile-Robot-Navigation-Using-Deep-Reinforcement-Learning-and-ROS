...
class EnvInterface(Node):
    def __init__(self):
        ...
        self.actio_space_sample_client = self.create_client(SampleActionSpace, "action_space_sample")
    ...
    def step(self, action):
        ...
        request.action = np.array([(action[0] + 1) / 2, action[1]], dtype=np.float32).tolist()
    ...
    def get_dimensions(self):
        ...
        while not self.dimensions_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("Service /get_dimensions not available, waiting again...")
        try:
    ...
    def sample_action_space(self):
        ...
        while not self.actio_space_sample_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("Service /action_space_sample not available, waiting again...")
        try:
