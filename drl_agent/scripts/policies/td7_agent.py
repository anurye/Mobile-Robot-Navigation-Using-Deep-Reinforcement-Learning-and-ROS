...
class Hyperparameters:
	...
	exploration_noise_min: float = 0.1
	exploration_noise_decay_steps: int = 5e5
...
class Agent(object):
	def __init__(self, state_dim, action_dim, max_action, offline=False, log_dir=None, hp=Hyperparameters()): 
		...
		self.actor_target = Actor(state_dim, action_dim, hp.zs_dim, hp.actor_hdim, hp.actor_activ).to(self.device)
		...
		self.critic_target = Critic(state_dim, action_dim, hp.zs_dim, hp.critic_hdim, hp.critic_activ).to(self.device)
		...
		self.fixed_encoder = Encoder(state_dim, action_dim, hp.zs_dim, hp.enc_hdim, hp.enc_activ).to(self.device)
		self.fixed_encoder_target = Encoder(state_dim, action_dim, hp.zs_dim, hp.enc_hdim, hp.enc_activ).to(self.device)

		self.checkpoint_actor = Actor(state_dim, action_dim, hp.zs_dim, hp.actor_hdim, hp.actor_activ).to(self.device)
		self.checkpoint_encoder = Encoder(state_dim, action_dim, hp.zs_dim, hp.enc_hdim, hp.enc_activ).to(self.device)
	...
	def select_action(self, state, use_checkpoint=False, use_exploration=True):
		with torch.no_grad():
			...
			if use_exploration:
				if self.hp.exploration_noise > self.hp.exploration_noise_min:
					self.hp.exploration_noise -= (1 - self.hp.exploration_noise_min)/self.hp.exploration_noise_decay_steps 
