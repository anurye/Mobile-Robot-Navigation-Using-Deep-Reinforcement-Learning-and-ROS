...
def load_trajectories(trajectories_file_path, start_goal_pairs_file_path, map_metadata):
    ...
    try:
        ...
        raise IOError(f"Unable to load start-goal pairs: {start_goal_pairs_file_path}: {e}")

    all_trajectories = []
    for i, trajectory in enumerate(trajectories):
        coords = [(point["x"], point["y"]) for point in trajectory]
        x_coords, y_coords = transform_coordinates(coords, map_metadata.origin, map_metadata.resolution, map_metadata.map_height)

        start, goal = start_goal_pairs[i]["start"], start_goal_pairs[i]["goal"]
        start_x, start_y = transform_coordinates((start["x"], start["y"]), map_metadata.origin, map_metadata.resolution, map_metadata.map_height)
        goal_x, goal_y = transform_coordinates((goal["x"], goal["y"]), map_metadata.origin, map_metadata.resolution, map_metadata.map_height)

        all_trajectories.append(Trajectory(x_coords, y_coords, start_x, start_y, goal_x, goal_y))
...
def plot_trajectory(ax, map_img, trajectories):
    ...
    for i, traj in enumerate(trajectories):
        ax.plot(traj.start_x, traj.start_y, marker="o", markersize=8, color="blue", label="Start" if i == 0 else "")
        ax.plot(traj.goal_x, traj.goal_y, marker="*", markersize=12, color="red", label="Goal" if i == 0 else "")
        ax.plot(traj.x_coords, traj.y_coords, label=f"Traj {i+1}")

def plot_comparison(map_file_name, trajectory_file_names, metrics_file_names, start_goal_pairs_file_name, drl_agent_src_path):
    ...
    for idx, (trajectory_file_name, metrics_file_name) in enumerate(zip(trajectory_file_names, metrics_file_names)):
        trajectories_file_path = os.path.join(test_runs_path, f"{trajectory_file_name}.json")
        ...
        trajectories = load_trajectories(trajectories_file_path, start_goal_pairs_file_path, map_metadata)
        metrics = load_metrics(metrics_file_path)

        plot_trajectory(axs[idx], map_img, trajectories)
        display_metrics(axs[idx], metrics)

        axs[idx].set_title(f"Trajectories: {trajectory_file_name}", fontsize=16, fontweight='bold')
        ...
        # Add a scale bar
        scalebar = ScaleBar(map_metadata.resolution, 'm', location='lower right',
...
def main():
    ...
    trajectory_file_sets = [["baseline_env_1_traj", "ours_env_1_traj"],
                            ["baseline_env_2_traj", "ours_env_2_traj"]]
    metrics_file_sets = [["baseline_env_1_metrics", "ours_env_1_metrics"],
                         ["baseline_env_2_metrics", "ours_env_2_metrics"]]
    start_goal_pairs_file_name = "test_config"

    for map_file_name, trajectory_files, metrics_files in zip(map_file_names, trajectory_file_sets, metrics_file_sets):
        plot_comparison(map_file_name, trajectory_files, metrics_files, start_goal_pairs_file_name, drl_agent_src_path)
